...
$(".savenext").click(function(){
	...
	if(validation((this.id).split('-')[1])) {
		...
		//alert('container:'+container_id);alert('pid:'+pid);alert('nid:'+nid);
		/*####################### END ################################################################################*/
...
function fillComp(id) {
	var sUrl = 'http://localhost/compare/index.php/agent/getCompaney?id='+id;
	$.ajax({
		url: sUrl,
		type:'POST',		
		//	dataType: 'json',
		...
		success: function(data){
			if(data=="error") {
				...
				var comp = JSON.parse(data);
					$('#companyName').html(comp.company);
					$('#companyDetail').html(comp.details);
					$('#companyImg').attr("src",'http://localhost/compare/admin/assets/'+(comp.logo));
...
function checkEmail(email) {
 if (/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(email)) {
	  var sUrl = 'http://localhost/compare/index.php/agent/checkEmail?email='+email;
	  //alert(sUrl);
	$.ajax({
		url: sUrl,
		type:'POST',		
...
function checkUsername(username) {
	  var sUrl = 'http://localhost/compare/index.php/agent/checkUsername?username='+username;
	$.ajax({
		url: sUrl,
		type:'POST',		
		//	dataType: 'json',
