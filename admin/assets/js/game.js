...
function removeClass() {
	...
	if(task=="add")
	$('.selectedClass').text('');
	$('.cellRow').removeClass('selectedClass');  getGameData();                      //remove all selected class
...
function rClass() {
	...
	cr='';cc='';ct='';c='';r='';cnt=0;mncc='';mncr='';mxcc='';mxcr='';   
	getGameData();                                        //set to initialize values in the all parameters
}
function selectedClass(r,c) {
	if(ct) {
		if(ct=='r') {
			...
			mncc = Number(minc);mxcc = Number(maxc);
			...
			mncr = Number(minc);mxcr = Number(maxc);
			//alert();
...
function validBlockLength() {
	var str = ($('#answerId').val());
	var len = '';
	len = (ct=='r')?(Number(mxcc) - Number(mncc) +1):(Number(mxcr) - Number(mncr) +1)
	if(len==str.length) {
		return true;
	}
	else {
	...
	$('#loader').html('<img style="margin-left: 230px;margin-top: 100px;margin-bottom: 110px;" src="/crosswords/admin/assets/images/ajax-loader2.gif"/>');
			...
			for (var i = 0; i< (grpData.length); i++) {
				...
				console.log(mncc+" "+mxcc);
					...
					$('#answerId').attr('maxlength',groupLength());
