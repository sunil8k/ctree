	...
	<?php $this->load->view("layout/left_content");?>
            ...
            <aside class="right-side">                
                ...
                <section class="content-header">
                    <h1>
                        Agents
                        <a href="<?= base_url();?>index.php/agent/addAgent?view=index" style="float: right; margin-right: 10px;">
                        ...
                        <div class="col-xs-12">
                            ...
                            <div class="box">
                                ...
                                <div class="alert alert-danger alert-dismissable" style="width:98%;">
                                        <i class="fa fa-check"></i>
                                        <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                                        <?php echo $this->session->flashdata('successful'); ?>
                                    </div>
                                    <?php } ?>
									  ...
									  <?php
                                          ...
                                          ?>                                       
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                			    <a title="View" onClick="profile(<?php echo $result->id; ?>)" data-toggle="modal" style="cursor:pointer;" data-target="#compose-modal"><i class="glyphicon glyphicon-search"></i></a>
                                               <?php if($this->session->userdata('username')=='admin'){ ?>
                                                <?php if($result->status==1){ ?>
                                                <a  href="<?= base_url();?>index.php/agent/active?id=<?php echo $result->id; ?>&view=index" onclick="return confirm('Are you sure you want to Un-publish ?');" title="Un-publish" ><i class="glyphicon glyphicon-ok-circle"></i></a>
                                                <?php }else { ?>
                                                <a href="<?= base_url();?>index.php/agent/active?id=<?php echo $result->id; ?>&view=index" onclick="return confirm('Are you sure you want to Publish ?');" title="Publish" ><i class="glyphicon glyphicon-ok-sign"></i></a>
                                                <?php } ?>
                                                <a  href="<?= base_url();?>index.php/agent/delete?del=<?php echo $result->id; ?>&view=index" onclick="return confirm('Are you sure you want to Delete ?');" title="Delete"><i class="glyphicon glyphicon-remove-circle"></i></a>
                                           		<a href="<?= base_url();?>index.php/agent/editAgent?id=<?php echo $result->id; ?>&view=index" onclick="return confirm('Are you sure you want to Update ?');" title="Update" ><i class="glyphicon glyphicon-pencil"></i></a> 
                                               <?php } ?>
                                                </td>
                                            </tr>
...
function profile(id)
{
	$('#datad').html('<img style="margin-left: 230px;margin-top: 100px;margin-bottom: 110px;" src="<?= base_url();?>assets/images/ajax-loader2.gif"/>');
		    $.ajax({
                        url: '<?= base_url();?>index.php/agent/view?id='+id,
                         type:'POST',
                       //	dataType: 'json',
                          error: function(){
